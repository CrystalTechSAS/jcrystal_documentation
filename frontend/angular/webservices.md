...
From the code-side of the app: you won't know, but if an error happens the end-user will see an alert on the app with the error returned by the server. 
...
Please remember that **all the examples presented here are from generated code from web services written on the backend**, this is only a guide for you to learn how to use similar webservices. 
...
Finally, let's review a web service that has multiple returns. Consider a website like Twitter, which has multiple users and multiple tweets, and think of a web service that given a word returns all the users and all the tweets related to that word. Therefore, the web service will receive one word and return the User entities and the Tweet entities that are related to that word, the signature of such web service would look like this on the frontend:

```typescript
...
export class ManagerTest{
	/**
	* /api/test/getUsersAndTweets
	**/
	static getUsersAndTweets(base : NetworkBase,word:string,onSuccess: (p0:UserNormal[], p1:TweetNormal[]) => void,onError : (error : RequestError)=>void = defaultOnError){
        ...
    }
    ...
```

As you can see, after the parameter `NetworkBase` there is a string parameter named `word`; also, the `onSuccess` is a function of type `(p0:UserNormal[], p1:TweetNormal[]) => void`, meaning that this web service returns an array of User entities with access level normal and another array of Tweets with access level normal.
...
export class MyComponent {
	constructor(public http: HttpClient) { }
...
    ManagerTest.getUsersAndTweets(this, "Escher", (users, tweets) => {
        //On success
    }, error => {
        //On error
    });
